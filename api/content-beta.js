module.exports = async function handler(req, res) {
  ...
  // UTM-campaign erottaa beta-klikkaukset prodista GA:ssa / GR:ssä
  const UTM = 'utm_source=gr&utm_medium=email&utm_campaign=AD.FIa-top5-beta';
  ...
  for (let slot = 1; slot <= 5; slot++) {
    ...
    products.push({
      slot,
      title:       get(cols, 1),
      productUrl:  get(cols, 3) + '?' + UTM + '&utm_content=slot' + slot,
      imageUrl:    get(cols, 4),
      rawPrice:    get(cols, 5),
      rawSale:     get(cols, 6),
      aiText:      get(cols, 9),
  ...
  // Beta-merkki — näkyy sähköpostissa jotta tiedät testailevasi betaa
  html += `
<table width="560" cellpadding="0" cellspacing="0" border="0" style="margin:0 auto;background:#222;">
  <tr>
    <td align="center" style="padding:4px 20px;">
      <p style="margin:0;font-size:11px;font-weight:bold;color:#fff;font-family:Arial,sans-serif;letter-spacing:2px;">⚠ BETA — layout-testi</p>
    </td>
  </tr>
</table>`;
